...
nums.push(55);
// The complexity of the push() operation will be O(1)O(1) since we only have to update the length of the array and add the new element.

// The pop() function #
// Returns and removes the element from the end of an array.
nums.pop();
// The complexity of the pop() operation will be O(1)O(1) since we only have to update the length of the array and remove the end index.
console.log(nums);
// Returns and removes the element from the beginning of the array. Try it out yourself!
nums.shift();
// The complexity of the shift() operation will be O(n)O(n) since we have to remove the element at index 0, shift all the elements to the left, and then update the length of an array.

// The unshift(value) function #
// Inserts the given element at the beginning of the array.
nums.unshift(99);
// The complexity of the unshift() operation will be O(n)O(n) since we have to update the length of an array, shift all the elements to the right, and then add the new element at index 0.
...
delete nums[2];
console.log(nums);
// The complexity of the delete() function will be O(n)O(n). To remove the first element from the beginning, we need to move all the following elements to the left.

// This function reverses an array. Use it like array.reverse(). inplace
nums.reverse();
// The complexity of the reverse() function will be O(n)O(n) since we have to traverse all the elements of an array.
console.log(nums);
...
// The complexity of the slice() function is O(n)O(n). Since slice() returns a new array from the start to the end. Therefore, in the worst-case, we have to make a copy of all the elements present in the array.
...
// The complexity of the concat() function will be O(m+n)O(m+n), where m is the size of the first array, and n is the size of the second array. concat() makes a new array of size m+n and then copy elements of both arrays one by one in a new array.
